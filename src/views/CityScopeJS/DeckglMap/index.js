...
export default function DeckGLMap() {
    ...
    useEffect(() => {
        console.log("cityIOdata changed:", cityIOdata);
        console.log("GEOGRID exists?", !!cityIOdata.GEOGRID);

        if (cityIOdata.GEOGRID) {
            console.log("Processing GEOGRID");
            const processed = processGridData(cityIOdata);
            console.log("First cell:", processed?.features?.[0]?.properties);
            setGEOGRIDDATA(processed);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [cityIOdata.GEOGRID]);
